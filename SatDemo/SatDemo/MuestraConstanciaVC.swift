...
class MuestraConstanciaVC: UIViewController {
    ...
    @IBAction func guardaConstancia(_ sender: AnyObject) {
    ...
    func presentaSubMenu(){
        ...
        let tomarFoto = UIAlertAction(title: "Descargar", style: .default, handler: {
            ...
            self.mensaje = "Se descargo tu Constancia"
            self.showAlert()

        })
        let libreria = UIAlertAction(title: "Enviar", style: .default, handler: {
            (alert: UIAlertAction!) -> Void in
            self.mensaje = "    Se envio tu Constancia"
            self.showAlert()
